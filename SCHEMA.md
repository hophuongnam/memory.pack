...
The stock system prompt's `metadata: { type: ... }` nested wrapper is **tolerated on read, never mutated**: both `index-memories.py` (via `k.strip()`) and `update-recall.mjs` (via a leading-whitespace-tolerant key regex) resolve indented children as recognized keys, so `type` and the rest parse correctly from flat OR nested OR `node_type`-bearing frontmatter. The recall hook edits **only** `recall_count`/`last_recalled`, in place, preserving every other byte (key order, the `metadata:` wrapper, indentation). It does **not** reshape files — there is no auto-flatten, by design (the harness's own output is not uniform, so normalizing-on-read was a fight with negative EV that reintroduced the silent-amnesia class; see the Evolution note). **There is no on-disk shape requirement — write whatever shape you get, and never reshape a file to satisfy a style rule.** The old "always write auto-memory frontmatter flat" instruction was **retired 2026-07-10**. It was never satisfiable: the CC harness's `Write` tool nests every memory it creates under its own `metadata:` wrapper, so honouring the rule meant a follow-up `Edit` on every single write, in every project, forever. It also protected nothing — no hook, indexer, or skill has ever read frontmatter with a column-0 anchor — while a same-day census of all 20 stores found 639 of 1517 memories (42%) already nested, with zero observed breakage.

**If you grep frontmatter, anchor tolerantly:** `grep -E '^[[:space:]]*type:'`, never `grep '^type:'` — on that same corpus the naive form saw 884 files of 1517. Everything above about the *engine* never reshaping remains true and is pinned by `tests/test_recall_frontmatter_preserve.mjs` + `tests/test_archive_resurrect_preserve.mjs`; the retirement itself is pinned by `tests/test_frontmatter_shape_agnostic.mjs`, which fails if any source file re-grows a column-0 read or if this document re-grows the retired instruction.
...
**`type` may sit at any indentation — flat or nested, both are canonical.** The stock Claude Code system prompt's `# auto memory` block shows a nested `metadata: { type }` shape, and the harness's `Write` tool produces exactly that; the harness also injects `node_type:` and `originSessionId:`. The engine tolerates all of it: both parsers resolve recognized keys at any indentation (`index-memories.py` via `k.strip()`, `update-recall.mjs` via a leading-whitespace-tolerant regex), and the recall hook **preserves whatever shape it finds** — it never reshapes a file (it edits only `recall_count`/`last_recalled` in place). So nested input does not break, is not "healed", and is **not drift**. Same applies to every other recognized field below.

### Harness-managed fields

- `originSessionId` — the Claude Code **harness itself** auto-appends this field on every Write/Edit to a memory file if the frontmatter doesn't already contain it, stamping the current session ID. It is NOT written by any hook in `Memory.Pack/hooks/`. (The gating mechanism is **unverified**: an earlier note cited a `/^originSessionId:/m.test` string found in the 2.1.x binary, but a 2026-07-10 census contradicts a column-0-anchored test — all 639 nested memories carry `originSessionId` *indented* inside the `metadata:` wrapper, and **not one** has picked up the duplicate column-0 key such a test would have appended. Don't rely on the regex; rely on the observed behaviour.) Consequences:
...
- **2026-07-10** — the **write-side** flat rule was retired too, closing this class. `SCHEMA.md` and `~/.claude/CLAUDE.md` had both instructed "always write frontmatter flat" and justified it with "the engine never reshapes, so a flat write stays flat". The first clause is true; the *inference* is not — the harness's `Write` tool nests before any hook runs, so a flat write never stayed flat (measured 2026-07-09 on 4 files, re-confirmed 2026-07-10). The rule was therefore unsatisfiable without an extra `Edit` per write and, worse, load-bearing for nothing: a census of all 20 stores found **639 of 1517 memories (42%) nested, 6 mixed, 0 broken**, and a source sweep found **no column-0 frontmatter read anywhere in `hooks/`, `index/`, or `skills/`** — `memory-lint` had already declared the nested wrapper "tolerated, not drift" back in May. The only casualty was the ad-hoc `grep '^type:'` the rule was meant to protect, which was already wrong about 42% of the corpus. New contract: **no on-disk shape requirement; grep with `^[[:space:]]*`; never flatten an existing file.** Pinned by `tests/test_frontmatter_shape_agnostic.mjs` (structural source scan + mutation check + a doc-regression guard on this file).
